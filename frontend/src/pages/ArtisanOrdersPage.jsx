...
export const ArtisanOrdersPage = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 space-y-6">
      {/* Header Banner */}
      <div className="bg-gradient-to-r from-stone-900 via-orange-950 to-stone-900 rounded-3xl p-6 sm:p-8 text-white border border-orange-800/40 shadow-xl space-y-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-2 border-t border-stone-800/80">
          <div className="p-3 rounded-2xl bg-white/5 border border-white/10">
            <span className="text-[10px] text-stone-400 font-bold uppercase block">Retail Orders</span>
            <span className="text-lg sm:text-xl font-black text-white">{orders.length}</span>
            <span className="text-[10px] text-orange-400 block font-semibold">{pendingRetailCount} In Progress</span>
          </div>

          <div className="p-3 rounded-2xl bg-white/5 border border-white/10">
            <span className="text-[10px] text-stone-400 font-bold uppercase block">Bulk B2B RFQs</span>
            <span className="text-lg sm:text-xl font-black text-white">{rfqs.length}</span>
            <span className="text-[10px] text-blue-400 block font-semibold">{pendingBulkCount} Open Inquiries</span>
          </div>

          <div className="p-3 rounded-2xl bg-white/5 border border-white/10">
            <span className="text-[10px] text-stone-400 font-bold uppercase block">Retail Value</span>
            <span className="text-lg sm:text-xl font-black text-emerald-400">₹{totalRetailRevenue.toLocaleString('en-IN')}</span>
            <span className="text-[10px] text-stone-400 block">Direct Artisan Payout</span>
          </div>

          <div className="p-3 rounded-2xl bg-white/5 border border-white/10">
            <span className="text-[10px] text-stone-400 font-bold uppercase block">Bulk Pipeline</span>
            <span className="text-lg sm:text-xl font-black text-cyan-400">₹{totalBulkDealValue.toLocaleString('en-IN')}</span>
            <span className="text-[10px] text-stone-400 block">GeM & Corporate B2B</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between gap-4 border-b border-stone-200 pb-3">
        <div className="flex items-center space-x-2">
          ...
          <button
            ...
            className={`flex items-center space-x-2 px-4 py-2.5 rounded-2xl font-extrabold text-xs sm:text-sm transition-all min-h-[44px] ${
              activeSubTab === 'retail'
          ...
          >
            ...
            <span className={`px-2 py-0.5 rounded-full text-[10px] font-black ${
              activeSubTab === 'retail' ? 'bg-white/20 text-white' : 'bg-orange-100 text-orange-800'
            }`}>
              {orders.length}
            </span>
          </button>
          ...
          >
            ...
            <span className={`px-2 py-0.5 rounded-full text-[10px] font-black ${
              activeSubTab === 'bulk' ? 'bg-white/20 text-white' : 'bg-blue-100 text-blue-800'
            }`}>
              {rfqs.length}
            </span>
          </button>
        ...
        <div className="space-y-4">
          {filteredOrders.length === 0 ? (
            <div className="py-16 text-center bg-white rounded-3xl border border-stone-200 p-8 space-y-3">
              ...
              <h3 className="text-base font-bold text-stone-800">No Retail Orders Received Yet</h3>
            ...
            <div className="grid grid-cols-1 gap-4">
              {filteredOrders.map((order) => {
                ...
                return (
                  <div
                    key={order.id}
                    className="bg-white rounded-3xl p-5 sm:p-6 border border-stone-200 shadow-sm space-y-4 hover:border-orange-300 transition-colors"
                  >
                    {/* Top Row: Order ID, Date, Status Badge */}
                    <div className="flex flex-wrap items-center justify-between gap-2 pb-3 border-b border-stone-100">
                      <div className="flex items-center space-x-2">
                        <span className="px-2.5 py-1 rounded-xl bg-orange-100 text-orange-800 font-mono font-bold text-xs">
                          {order.id}
                        </span>
                      ...
                      <div className="flex items-center space-x-2">
                        <span className="px-2.5 py-0.5 rounded-full bg-stone-100 text-stone-700 font-bold text-[10px] uppercase">
                          👤 Retail Consumer
                        </span>
                        <span className={`px-3 py-1 rounded-full text-xs font-black ${
                          isDelivered ? 'bg-emerald-100 text-emerald-800' :
                          isShipped ? 'bg-blue-100 text-blue-800' :
                          isPacked ? 'bg-purple-100 text-purple-800' :
                          isConfirmed ? 'bg-amber-100 text-amber-800' :
                      ...
                      <div className="md:col-span-6 flex items-center space-x-4">
                        ...
                        <div className="space-y-1">
                          ...
                          <div className="text-xs text-stone-600 font-medium">
                            <span>Quantity: <b>{order.qty || 1} Unit(s)</b></span> • <span>Price: ₹{order.price?.toLocaleString('en-IN')}</span>
                          </div>
                    ...
                    <div className="pt-2 flex flex-wrap items-center justify-between gap-3 border-t border-stone-100">
                      <div className="text-xs text-stone-500 font-medium">
                        Logistics: <span className="font-bold text-stone-700">IndiaPost Dak Ghar Niryat Kendra</span>
                      </div>
                      ...
                      <div className="flex items-center space-x-2">
                        {isPlaced && (
                          <button
                            ...
                            className="px-4 py-2 rounded-xl bg-orange-600 hover:bg-orange-500 text-white font-bold text-xs shadow-md transition-all active:scale-95 flex items-center space-x-1.5"
                          >
                        ...
                        {isConfirmed && (
                          <button
                            ...
                            className="px-4 py-2 rounded-xl bg-purple-600 hover:bg-purple-500 text-white font-bold text-xs shadow-md transition-all active:scale-95 flex items-center space-x-1.5"
                          >
                        ...
                        {isPacked && (
                          <button
                            ...
                            className="px-4 py-2 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-bold text-xs shadow-md transition-all active:scale-95 flex items-center space-x-1.5"
                          >
        ...
        <div className="space-y-4">
          {rfqs.length === 0 ? (
            <div className="py-16 text-center bg-white rounded-3xl border border-stone-200 p-8 space-y-3">
              ...
              <h3 className="text-base font-bold text-stone-800">No B2B Bulk Inquiries Yet</h3>
              <p className="text-xs text-stone-500 max-w-md mx-auto">
            ...
            <div className="grid grid-cols-1 gap-4">
              {rfqs.map((rfq) => {
                ...
                return (
                  <div
                    key={rfq.id}
                    className="bg-white rounded-3xl p-5 sm:p-6 border border-stone-200 shadow-sm space-y-4 hover:border-blue-300 transition-colors"
                  >
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-2 pb-3 border-b border-stone-100">
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        <span className={`px-3 py-1 rounded-full text-xs font-black ${
                          ...
                          isQuoted ? 'bg-purple-100 text-purple-800' :
                    ...
                    <div className="space-y-1.5">
                      ...
                      <p className="text-xs text-stone-800 bg-blue-50/60 p-3.5 rounded-2xl border border-blue-100 leading-relaxed font-medium">
                        <b>{rfq.category}:</b> {rfq.requirements}
                      </p>
                    </div>

                    {/* Bulk Numbers Strip */}
                    <div className="grid grid-cols-3 gap-3 p-3 rounded-2xl bg-stone-50 border border-stone-200 text-xs">
                      <div>
                        <span className="text-[10px] text-stone-400 font-bold uppercase block">Bulk Quantity</span>
                        <span className="font-extrabold text-stone-900 text-sm">{rfq.quantity} Units</span>
                      </div>
                      <div>
                        <span className="text-[10px] text-stone-400 font-bold uppercase block">Target Price / Unit</span>
                        <span className="font-extrabold text-emerald-700 text-sm">₹{rfq.target_price?.toLocaleString('en-IN')}</span>
                      </div>
                      <div>
                        <span className="text-[10px] text-stone-400 font-bold uppercase block">Total Contract Value</span>
                        <span className="font-black text-blue-900 text-sm">₹{dealValue.toLocaleString('en-IN')}</span>
                      </div>
                    </div>
                        ...
                        {!isAccepted && (
                          <>
                            <button
                              ...
                              className="px-3.5 py-2 rounded-xl bg-stone-100 hover:bg-stone-200 text-stone-800 font-bold text-xs transition-colors active:scale-95"
                            >
