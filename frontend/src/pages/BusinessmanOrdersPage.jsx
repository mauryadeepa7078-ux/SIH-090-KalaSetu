    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8 space-y-6 sm:space-y-8 animate-fade-in pb-28">
      ...
      <div className="bg-gradient-to-r from-stone-900 via-blue-950 to-stone-900 rounded-[28px] p-6 sm:p-8 text-white border border-blue-800/40 shadow-xl space-y-4 relative overflow-hidden">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 relative z-10">
          <div className="space-y-1.5">
            <div className="inline-flex items-center space-x-2 px-3 py-1 rounded-full bg-blue-500/20 text-blue-300 border border-blue-500/30 text-xs font-bold font-sans">
          ...
          <div className="flex items-center space-x-2 self-start sm:self-center font-sans">
            <button
              onClick={fetchRfqs}
              disabled={loading}
              className="px-4 py-2.5 rounded-2xl bg-stone-800 hover:bg-stone-750 text-stone-200 border border-stone-700 text-xs font-bold flex items-center space-x-1.5 transition-all active:scale-95"
            >
            ...
            <button
              ...
              className="px-4 py-2.5 rounded-2xl bg-blue-600 hover:bg-blue-500 text-white font-bold text-xs shadow-md transition-all active:scale-95"
            >
              + New RFQ
            </button>
          </div>
        ...
        {/* Metrics Strip */}
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-3 border-t border-stone-800/80 relative z-10 font-sans">
          ...
          <div className="p-3.5 rounded-2xl bg-white/5 border border-white/10">
            ...
            <span className="text-lg sm:text-xl font-black text-cyan-400 font-serif">₹{totalProcurementValue.toLocaleString('en-IN')}</span>
          ...
          <div className="p-3.5 rounded-2xl bg-white/5 border border-white/10">
            ...
            <span className="text-lg sm:text-xl font-black text-emerald-400 font-serif">100% Secure</span>
          ...
          <div className="p-3.5 rounded-2xl bg-white/5 border border-white/10">
            ...
            <span className="text-lg sm:text-xl font-black text-white font-serif">IndiaPost DNK</span>
      ...
      {rfqs.length === 0 ? (
        <div className="text-center py-20 bg-white dark:bg-stone-900 rounded-[28px] border border-stone-200/80 dark:border-stone-800 p-8 space-y-4 shadow-card">
          <FileText className="w-16 h-16 text-stone-300 dark:text-stone-600 mx-auto" />
          ...
          <button
            onClick={() => setActiveTab('businessman-home')}
            className="px-6 py-3 rounded-2xl bg-blue-600 hover:bg-blue-500 text-white font-extrabold text-xs shadow-md transition-all active:scale-95 font-sans"
          >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8 items-start">
                ...
                return (
                  <div
                    ...
                    className={`p-4 sm:p-5 rounded-[24px] border transition-all cursor-pointer shadow-sm ${
                      isSelected
                        ? 'bg-blue-50/80 dark:bg-blue-950/40 border-blue-500 shadow-md ring-2 ring-blue-500/30'
                        : 'bg-white dark:bg-stone-900 border-stone-200/80 dark:border-stone-800 hover:border-blue-300 dark:hover:border-blue-700'
                    }`}
                  >
                    <div className="space-y-2">
                      <div className="flex items-center justify-between">
                        <span className="font-mono text-[10px] font-bold px-2 py-0.5 rounded bg-blue-100 dark:bg-blue-900 text-blue-900 dark:text-blue-200">
                          {r.id}
                        </span>
                        <span className={`text-[10px] px-2.5 py-0.5 rounded-full font-black ${
                          r.status === 'ACCEPTED' ? 'bg-emerald-100 dark:bg-emerald-950/60 text-emerald-800 dark:text-emerald-300 border border-emerald-300 dark:border-emerald-800' :
                          r.status === 'QUOTED' ? 'bg-purple-100 dark:bg-purple-950/60 text-purple-800 dark:text-purple-300 border border-purple-300 dark:border-purple-800' :
                          r.status === 'IN_PRODUCTION' ? 'bg-cyan-100 dark:bg-cyan-950/60 text-cyan-800 dark:text-cyan-300 border border-cyan-300 dark:border-cyan-800' :
                      ...
                      <div>
                        <h4 className="font-extrabold text-sm text-stone-900 dark:text-white line-clamp-1 font-serif">
                          {r.category || 'Handicraft Bulk Order'}
                        </h4>
                        <p className="text-xs text-stone-500 dark:text-stone-400 line-clamp-1 mt-0.5 font-sans">
                          {r.organization || r.buyer_name} • {r.quantity} Units
                        </p>
                      </div>

                      <div className="flex items-center justify-between pt-2 border-t border-stone-100 dark:border-stone-800 text-xs font-sans">
          ...
          {selectedRfq && (
            <div className="lg:col-span-7 bg-white dark:bg-stone-900 rounded-[28px] p-6 sm:p-8 border border-stone-200/80 dark:border-stone-800 shadow-card space-y-6">
              ...
              <div className="flex flex-wrap items-center justify-between pb-4 border-b border-stone-100 dark:border-stone-800 gap-3">
                <div className="space-y-0.5">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-xs px-2.5 py-0.5 rounded-full bg-blue-50 dark:bg-blue-950/50 text-blue-700 dark:text-blue-300 font-bold border border-blue-200 dark:border-blue-800 font-sans">
              ...
              <div className="p-4 rounded-2xl bg-blue-50/80 dark:bg-blue-950/30 border border-blue-100 dark:border-blue-900/40 space-y-2 text-xs font-sans">
                ...
                <div className="flex items-center justify-between pt-2 border-t border-blue-200/60 dark:border-blue-900/40 text-[11px] text-stone-500 dark:text-stone-400">
                  ...
                  {rfqStages.map((stage, idx) => {
                    ...
                    return (
                      <div key={stage.key} className="relative flex items-start space-x-3">
                        ...
                        {/* Stage Content */}
                        <div className={`flex-1 transition-all ${isCurrent ? 'p-3.5 rounded-2xl bg-blue-50 dark:bg-blue-950/40 border border-blue-200 dark:border-blue-900/50' : ''}`}>
                          <div className="flex items-center space-x-2">
                            <span className={`text-xs sm:text-sm font-extrabold ${isCurrent ? 'text-blue-900 dark:text-blue-300' : isCompleted ? 'text-stone-900 dark:text-white' : 'text-stone-400 dark:text-stone-600'}`}>
              ...
              <div className="pt-4 border-t border-stone-100 dark:border-stone-800 flex flex-wrap items-center justify-between gap-3 font-sans">
                <div className="flex items-center space-x-2">
                  <button
                    onClick={() => showToast('Downloading HSN & GeM Compliance Contract...', 'success')}
                    className="px-4 py-2.5 rounded-xl bg-stone-100 dark:bg-stone-800 hover:bg-stone-200 dark:hover:bg-stone-700 text-stone-800 dark:text-stone-200 text-xs font-bold flex items-center space-x-1.5 transition-colors"
                  >
